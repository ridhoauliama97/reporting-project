...
- Date range filter (via `DateFilter`) drives every page's widgets/table/chart; filtering uses `filterByDateRange()` (defaults to `purchaseDate`, some pages pass `poDate`/`prDate`). `DateFilter` commits immediately on every change (native date input only emits empty or full dates, so no keystroke spam); **range state is per-route, not global** — the app keeps `rangesByPath` (keyed by `location.pathname`) in `AppRoutes`, so each page keeps its own range and defaults to "Semua Data" (`{start:null, end:null}`), never inherited from another page (App holds the `onDateRangeChange` per page that writes into `rangesByPath`). Label is honest for partial/ranges: both set = "d MMM – d MMM", start-only = "Mulai …", end-only = "Sampai …", none = "Semua Data", reversed = "Rentang tidak valid". No auto-swap: a reversed range applies as typed and shows a hint text; a **Reset** button (appears when either field is filled) clears both back to "Semua Data". Empty string still parses to `null` and inclusive end is normalized to 23:59:59.999 via `parseDateInput` + the commit normalizer in DateFilter.

## App State / Data Flow (Refactor 2026-09)

- `App.tsx` is now a **container**: loads data (Web Worker), holds loading/error/retry + `createAuthClient` session, and renders `Router` → `/docs`, `/login`, `/reset-password`, `/settings` and `path="*"` → `<AppRoutes …/>`. `AppRoutes` (a component INSIDE `Router`, so `useLocation` is legal) owns `rangesByPath` and all the `filtered*` `useMemo`s, then renders the inner routes; it receives `items`, `poItems`, `filteredTransfers/filteredAdjustments/filteredUsages`, `allStock` from App. Null-range fallback uses a module-level `NULL_RANGE` const so `useMemo` deps stay stable.
