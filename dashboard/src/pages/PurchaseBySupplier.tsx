...
export default function PurchaseBySupplier({ items, dateRange, onDateRangeChange }: PurchaseBySupplierProps) {
  ...
  const avgPerSupplier = supplierData.length > 0 ? grandTotal / supplierData.length : 0;
  ...
  const columns = [
    ...
    { key: 'totalQuantity', label: 'Total Kuantitas', align: 'right' as const, sortable: true, render: (item: SupplierData) => formatNumber(item.totalQuantity) },
    { key: 'totalPurchase', label: 'Total Pembelian', align: 'right' as const, sortable: true, render: (item: SupplierData) => formatRupiah(item.totalPurchase) },
    { key: 'percentOfGrand', label: '% dari Grand Total', align: 'right' as const, sortable: true, render: (item: SupplierData) => formatPercent(item.percentOfGrand) },
  ];

  return (
    <PageLayout title="Pembelian per Supplier" dateRange={dateRange} onDateRangeChange={onDateRangeChange}>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Pembelian" value={formatRupiah(grandTotal)} />
        <StatCard title="Jumlah Supplier Aktif" value={formatNumber(supplierData.length)} />
        <StatCard title="Rata-rata per Supplier" value={formatRupiah(avgPerSupplier)} />
        <StatCard
